...
from ..core.models import (
    CountryInputRecord, CountryExtractedData, CountryConfidenceScores, TransparencyMetadata
)
...
class CountryExtractionEngine:
    ...
    def __init__(
        ...
        # Output directory
        self.output_dir = Path(config.get('output', {}).get('directory', 'output/country_extracted'))
    ...
    def _load_field_options(self) -> Dict[str, List[str]]:
        ...
        if fields_file.exists():
            with open(fields_file, 'r') as f:
                ...
                return {
                    'country': [str(v) for v in (data.get('country') or [])]
                }
    ...
    async def extract_from_record(
        ...
        try:
            ...
            affiliation_text = record.first_author_affiliation or record.full_author_text or ""
            ...
            country = self._normalize_choice(extracted.get('country'), self.field_options['country'])
    ...
    def _build_prompt(self, author_text: str, affiliation_text: str) -> str:
        ...
        country_options = "\n  - ".join([f'"{opt}"' for opt in self.field_options['country']])
    ...
    def save_results_to_csv(self, results: List[CountryExtractedData], output_path: Path):
